...
const showData = ({data, next, prev}) => {
    ...
    <ul class="songs">
    ${
       data.map((song) => `<li><span><strong>${song.artist.name}</strong> - ${song.title}</span><button class="btn" data-artist="${song.artist.name}" data-songtitle="${song.title}">Letra</button></li>`).join("")
    }
    </ul>
    `
    if(prev || next) {
